...
const config = {
    // メンテナンス期間（日本時間）
    maintenance: {
      startTime: '2025-09-09T22:45:00', // 開始時刻
      endTime: '2025-09-09T22:55:00',   // 終了時刻
    },
    // ページ自動更新（秒）
    // 0に設定すると自動更新を無効化
    autoRefreshSeconds: 300, // 5分
    // サービス名
    serviceName: 'misskey.vip',
  };
  
  // ==================================================================
  // Cloudflare Snippet エントリーポイント
  // ==================================================================
  export default {
    async fetch(request) {
      const maintenancePage = generateMaintenanceHTML(config);
  ...
  function generateMaintenanceHTML(config) {
    const { maintenance, autoRefreshSeconds, serviceName } = config;
    const startTimeISO = `${maintenance.startTime}+09:00`;
    const endTimeISO = `${maintenance.endTime}+09:00`;
  
    const autoRefreshTag = autoRefreshSeconds > 0
  ...
  <head>
      ...
      <title>メンテナンス中 | ${serviceName}</title>
      ${autoRefreshTag}
  ...
  <body>
      ...
      <script>
          (function() {
              // 設定値
              const startTimeISO = '${startTimeISO}';
              const endTimeISO = '${endTimeISO}';
              ...
              // メンテナンス期間の表示
              document.getElementById('start-time').textContent = formatDateTime(startTimeISO);
              document.getElementById('end-time').textContent = formatDateTime(endTimeISO);
  
              // カウントダウン処理
              const countDownDate = new Date(endTimeISO).getTime();
              ...
              const countdownFunction = setInterval(function() {
                  ...
                  if (distance < 0) {
                      ...
                      countdownContainer.innerHTML = "<p>サービスを再開しました。ページを再読み込みしてください。</p>";
                      return;
                  }
